...
- 当任务仍在运行或被中断、`briefs/index.html` / `index.html` 尚未生成时：
  - “发给师兄”模块不再显示可复制的中文简报/批量报告链接。
  - 推荐下一步里的中文简报/批量报告入口改为等待生成卡片，避免点击后 404。
- 增加服务器环境识别：
  - CLI 在服务器上生成的相对路径 job 也能显示公开分享链接。
- 已将被服务重启中断的任务 `20260805-154447-56sfq` 标记为失败，并生成新的可分享任务：
  - `20260805-155728-3z2zr`
